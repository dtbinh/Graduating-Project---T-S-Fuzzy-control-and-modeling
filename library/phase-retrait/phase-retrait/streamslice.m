function [ret1, ret2] = streamslice(varargin)
    ...
    %   $Revision: 1.6.4.6 $  $Date: 2012/07/05 16:47:35 $
    ...
    if isempty(w)  % 2D
        ...
        % xyuvcheck -> xyzchk
        [msg, x, y] = xyzchk(x,y,u,v);  error(msg) 
        [ny,nx] = size(u);
        if isempty(x)
        ...
        %xyzuvwcheck -> xyzvchk
        [msg, x, y, z] = xyzvchk(x,y,z,u,v,w);  error(msg)         
        ...
        for j = 1:length(sx)
            if ~( anynan(wi(:,j,:)) || anynan(vi(:,j,:)) )
                [verts, averts] = nicestreams(...
                    reshape(zi(:,j,:),[ny nz]),...
                    reshape(yi(:,j,:),[ny nz]),...
                    reshape(wi(:,j,:),[ny nz]),...
                    reshape(vi(:,j,:),[ny nz]),density,arrows);
        ...
        for j = 1:length(sy)
            if ~( anynan(wi(j,:,:)) || anynan(ui(j,:,:)) )
                [verts, averts] = nicestreams(...
                    reshape(zi(j,:,:),[nx nz]),...
                    reshape(xi(j,:,:),[nx nz]),...
                    reshape(wi(j,:,:),[nx nz]),...
                    reshape(ui(j,:,:),[nx nz]),density,arrows);
                for k = 1:length(verts)
                    vv = verts{k};
                    if ~isempty(vv)
        ...
        for j = 1:length(sz)
            if ~( anynan(ui(:,:,j)) || anynan(vi(:,:,j)) )
                [verts, averts] = nicestreams(xi(:,:,j),yi(:,:,j),ui(:,:,j),vi(:,:,j),density,arrows);
                for k = 1:length(verts)
                    vv = verts{k};
                    if ~isempty(vv)
...
function [vertsout, avertsout] = nicestreams(x,y,u,v,density,arrows)
    ...
    nrstart = ceil(num*density);
    ncstart = ceil(num*density);
    nrend   = ceil(num*density*4);
    ncend   = ceil(num*density*4);
    nrarrow = ceil(num*density);
    ncarrow = ceil(num*density);
...
function [x, y, z, u, v, w, sx, sy, sz, density, arrows, method] = parseargs(nin, vargin)
    ...
    [x, y, z, w, sx, sy, sz, density, arrows, method] = deal([]);
    
    for j=1:2
        if nin>0
            lastarg = vargin{nin};
            if ischar(lastarg) % streamslice(...,'method'),  streamslice(...,'noarrows')
                if ~isempty(lastarg)
    ...
    elseif nin==9 || nin==10   % streamslice(x,y,z,u,v,w,sx,sy,sz) or streamslice(x,y,z,u,v,w,sx,sy,sz,density)
        ...
        error(message('MATLAB:streamslice:WrongNumberOfInputs'));
    end
